/**
 * The "In short" panel for the agreement.
 *
 ...
 *
 *   1. **Tied to one agreement version.** The list is named for the version whose text
 *      it describes, and a new version gets its own list rather than inheriting this
 *      one — a renumbered or reworded clause makes the summary a lie.
 *   2. **Quote the document, don't improve it.** Where the agreement is weak or
 *      self-contradictory — §36.1 grants a *request* to exit, not a right — the summary
 *      says so. A summary that reads better than the contract is worse than no summary,
 *      because it becomes the thing the gym relied on.
 *   3. **Figures are literals, matching the version's frozen text.** Deliberately
 *      not interpolated from `PARTNERSHIP`: the clauses transcribe "15,000" and
...
export type PlainLanguageItem = {
   ...
   * Asserted by `client/src/__tests__/shared/agreement-v2-3.test.ts`.
   */
  quotes?: string[];
};
 ...
 *
 * Eleven items are the terms that bind and bite. The twelfth is §47: how you sign, and
 * what we keep as proof of it. It is not a term that binds, and it is here anyway, because
 * the panel is what a non-lawyer relies on when they click "I have read and agree" — and
 * "clicking this is the signature" is the one thing about the mechanics they need to have
 * been told.
 *
 * §5.9's GST treatment is deliberately *not* here. It is favourable, it is administrative,
 * and twelve lines is already at the edge of what somebody reads before scrolling.
 */

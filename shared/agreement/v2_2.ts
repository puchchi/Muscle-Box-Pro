/**
 * MuscleBoxPro Machine Placement & Profit Sharing Agreement, version 2.2.
 *
 * This is the version that gets issued. v2.1 stays in the repository as the faithful
 * transcription of the source PDF and is never edited — the version string is part of
 * the hash stored with a signature, so a resolved clause means a new version file, not
 * a correction to an old one.
 *
 * ── What 2.2 is ──────────────────────────────────────────────────────────────
 * 2.1 carried eight `blocks-send` markers and five items awaiting outside sign-off.
 * Every one of the eight is closed here, from decisions recorded in
 * docs/gym-onboarding.md §12. The clause text for those items is drafted in-house
 * rather than transcribed, which is the material difference between this file and
 * v2_1.ts: 2.1 could be wrong only by mis-transcription, 2.2 can be wrong by drafting.
 * The cover therefore carries a standing `needs-review` marker saying so, and it does
 * not clear until counsel, a CA and a food-law consultant have read it.
 *
 * ── Rules (unchanged from 2.1) ───────────────────────────────────────────────
 *   - FROZEN the moment one gym signs against it. Any content change means v2_3.ts.
...
export const AGREEMENT_V2_2: Agreement = {
  version: "2.2",
  title: "Machine Placement & Profit Sharing Agreement",
  subtitle: "MUSCLEBOXPRO",
  runningFooter: "MuscleBoxPro - Machine Placement & Profit Sharing Agreement - Version 2.2",
  ...
  cover: [
    ...
    {
      kind: "todo",
      id: "v2-2-not-reviewed-by-counsel",
      severity: "needs-review",
      problem:
        "The clauses that 2.2 resolves — §5.9 (GST on the deposit), §6.1 and §6.3 (the milestone, its interpretation and the one-way ratchet), §24.6 (MuscleBoxPro as FSSAI Food Business Operator), §34 (liability, deliberately uncapped), §36.2 with Schedule B (early termination at Nil), and §46 (courts at Gautam Buddha Nagar, no arbitration) — were settled as commercial decisions and drafted in-house. No Indian legal counsel, CA or food-law consultant has read them. Separately, this Agreement is issued unstamped by decision: removing the stamp-duty note removed the reminder, not the liability, and an unstamped instrument can be inadmissible in evidence until stamped with penalty.",
      resolution:
        "Have counsel review §§33, 34, 36 and 46; a CA confirm §5.9 and the stamp position for the state of execution; a food-law consultant confirm §24.6 and Schedule F. Then either delete this marker or issue v2_3.ts with their wording. This is a needs-review marker and so does not block issuing — proceeding is a knowing decision to carry the risk, not an oversight.",
    },
  ],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ["Gym Representative", "[to be completed at installation]"],
            ["MuscleBoxPro Representative", "[to be completed at installation]"],
          ],
        },
        {
          kind: "checklist",
...
/**
 * What changed from 2.1, keyed by the marker id it closes.
 *
 * Kept in the code rather than only in docs/gym-onboarding.md because the next person
 * to touch this file will be reading the file, not the doc. Every id here was a
 * `blocks-send` or `needs-review` marker in v2_1.ts; the test file asserts that none of
 * them still appears in 2.2's tree, which is what stops a marker being deleted without
 * the clause behind it being fixed.
 */
export const AGREEMENT_V2_2_RESOLUTIONS: Readonly<Record<string, string>> = {
  "cover-version-mismatch": "Cover, footer and version string all read 2.2.",
  "cover-execution-note-unresolved":
    "The execution note is removed. Stamp duty is not mentioned: the agreement is issued unstamped by decision. Tax, FBO, liability and forum are settled in §§5.9, 24.6, 34 and 46.",
  "s4-4-empty": "Clause 4.4 deleted. Nothing renumbers — it was the last clause in §4.",
  "s5-1-amount-in-words":
    "§5.1 renders {{securityDepositInWords}}, produced by rupeesInWords() from the same integer as the figure. A negotiated deposit can no longer disagree with its own words.",
  "s6-1-no-heading":
    '§6.1 is headed "Profit-Sharing Milestone" and states the milestone as clause text. The "UPDATED COMMERCIAL MILESTONE" callout is gone.',
  "s6-1-mojibake":
    "Written in TypeScript source with real ₹ and no bullet glyphs, so there is nothing to corrupt.",
  "s6-milestone-ambiguity":
    "The second test is ₹5,00,000 of cumulative Net Profit as §7 defines it, not cumulative gross. Stated identically in §6.1, §6.3, §21.5, Schedule B and Schedule C step 4.",
  "s6-3-empty":
    "§6.3 has five sub-clauses: per-machine assessment and no reset on relocation (6.3.1), Net Profit means the pool not the share (6.3.2), the mid-month split (6.3.3), the one-way ratchet (6.3.4), and MBP's obligation to record and show the crossing point (6.3.5).",
  "s10-3-table-truncated":
    'A "5,000 and above" row was added and §10.3 now says the table illustrates the rule rather than capping it.',
  "s21-5-revenue-milestone-omitted":
    "§21.5 preserves the cup count, the cumulative Net Profit figure and the Milestone, and points at §6.3.1.",
  "s24-6-fbo-unresolved":
    "MuscleBoxPro is the FSSAI Food Business Operator for the dispensed food and holds the licence at its own cost. Recorded in §24.6, Schedule B and Schedule F.",
  "s33-3-indemnity-not-final":
    "§33.3 deleted. §33.2 gained a food-safety indemnity in the gym's favour, which is the risk it cannot control and we can.",
  "s34-liability-cap-undetermined":
    "§34 rewritten as four self-contained clauses: consequential loss excluded, no monetary cap on direct loss, the gym's profit share expressly outside the exclusion, and non-excludable liability preserved.",
  "s46-dispute-mechanism-missing":
    "§46 gives governing law, a 30-day escalation, and the exclusive jurisdiction of the courts at Gautam Buddha Nagar. No arbitration, stated in terms so the omission is not read as an oversight.",
  "schedule-b-unlabelled-ratio-rows":
    'The three unlabelled rows are replaced by "Profit Share, before/after the Milestone" and an explicit Milestone row. FBO and forum rows added.',
  "schedule-b-early-termination-charge":
    "Nil, conditional on §36.1's 30 days' written notice. §36.2 now states the position itself instead of pointing at a schedule row.",
  "schedule-c-step4-conflicts-with-s6-1":
    "Step 4 states the same earlier-of test as §6.1 and refers to §6.3 for how it is applied. A worked example was added.",
} as const;

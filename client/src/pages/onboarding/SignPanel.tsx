 ...
 * The signing panel.
 *
 * Two phases, deliberately: identity and assent first, then a code emailed to the
 * §41 notices address. The link in the email proves control of that address right up
 * until someone forwards it; a code requested at the moment of signing proves it
 * again, at that moment. One extra screen for a materially stronger audit trail on a
 * 24-month agreement — see docs/gym-onboarding.md §3, step 3.
 *
 ...
 * which is the content hash plus the OTP plus the server timestamps.
 */
export default function SignPanel({
  ...
  /** Null while the browser is still hashing the rendered document. */
  contentHash: string | null;
  hasReadToEnd: boolean;
  /** Set when the agreement may not be issued at all. Overrides everything below. */
  ...
  onSign(input: { fullName: string; designation: string; otpCode: string }): Promise<boolean>;
}) {
  ...
  async function sign() {
    ...
    setErrors({});
    // The panel passes back only what it collected. The hash it was handed is the
    // one that goes on the record, so this component never recomputes it.
    if (!(await onSign({ fullName, designation, otpCode }))) setFailed(true);
  }
